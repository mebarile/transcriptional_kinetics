function main_model7(n,reg,pop1,pop2,model)

addpath(genpath('./matlab_simulations/'))

rng(1)

filename = ['./data_output/parameters_' num2str(n) '_' num2str(model) '_model7_reg' num2str(reg) '_' num2str(pop1) '_' num2str(pop2) '.mat'];
...
modelfun = @ simulate_kinetics_model7;
...
data4 = load(['./data_output/parameters_' num2str(n) '_' num2str(model)  '_model4_reg' num2str(reg) '_' num2str(pop1) '_' num2str(pop2) '.mat']);
...
objectiveFunction = @(theta) llKin_model7(theta,modelfun,D,pop1,pop2,options);
