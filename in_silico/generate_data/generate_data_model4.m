function generate_data_model4
%% This code generates the genes whose kinetic rates follow Model 4 rules.
...
for gene = 1:100
    ...
    theta(1:8:32) = rand(4,1);
    theta(2:8:32) = randn(4,1) .* abs(theta(1:8:32)) * width + theta(1:8:32) .* interval(sign_temp);
    theta(3:8:32) = randn(4,1) .* abs(theta(2:8:32)) * width + theta(2:8:32) .* interval(sign_temp);
    theta(4:8:32) = randn(4,1) .* abs(theta(3:8:32)) * width + theta(3:8:32) .* interval(sign_temp);
    theta(5:8:32) = randn(4,1) .* abs(theta(4:8:32)) * width + theta(4:8:32) .* interval(sign_temp);
    theta(6:8:32) = randn(4,1) .* abs(theta(5:8:32)) * width + theta(5:8:32) .* interval(sign_temp);
    theta(7:8:32) = randn(4,1) .* abs(theta(6:8:32)) * width + theta(6:8:32) .* interval(sign_temp);
    theta(8:8:32) = randn(4,1) .* abs(theta(7:8:32)) * width + theta(7:8:32) .* interval(sign_temp);

    %% generate the error on the counts

    theta(25+8) = log(.1+3*rand);
    theta(26+8) = log(2+2*rand);
    theta(27+8) = log(.1+3*rand);
    theta(28+8) = log(2+2*rand);


    %% generate the spliced and unspliced counts

    [~,~,~,N,~,~] = simulate_kinetics_model4(mean_time,theta,[],[],[]);
