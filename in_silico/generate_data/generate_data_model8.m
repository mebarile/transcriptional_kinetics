function generate_data_model8
%% This code generates the genes whose kinetic rates follow Model 8 rules.
...
for gene = 1:100
    ...
    theta(1:8:48) = rand(6,1);
    theta(2:8:48) = randn(6,1) .* abs(theta(1:8:48)) * width + theta(1:8:48) .* interval(sign_temp);
    theta(3:8:48) = randn(6,1) .* abs(theta(2:8:48)) * width + theta(2:8:48) .* interval(sign_temp);
    theta(4:8:48) = randn(6,1) .* abs(theta(3:8:48)) * width + theta(3:8:48) .* interval(sign_temp);
    theta(5:8:48) = randn(6,1) .* abs(theta(4:8:48)) * width + theta(4:8:48) .* interval(sign_temp);
    theta(6:8:48) = randn(6,1) .* abs(theta(5:8:48)) * width + theta(5:8:48) .* interval(sign_temp);
    theta(7:8:48) = randn(6,1) .* abs(theta(6:8:48)) * width + theta(6:8:48) .* interval(sign_temp);
    theta(8:8:48) = randn(6,1) .* abs(theta(7:8:48)) * width + theta(7:8:48) .* interval(sign_temp);

    %% generate the error on the counts

    theta(25+24) = log(.1+3*rand);
    theta(26+24) = log(2+2*rand);
    theta(27+24) = log(.1+3*rand);
    theta(28+24) = log(2+2*rand);


    %% generate the spliced and unspliced counts

    [~,~,~,N,~,~] = simulate_kinetics_model8(mean_time,theta,[],[],[]);
