function generate_data_model7
%% This code generates the genes whose kinetic rates follow Model 7 rules.
...
for gene = 1:100
    ...
    theta(1:8:40) = rand(5,1);
    theta(2:8:40) = randn(5,1) .* abs(theta(1:8:40)) * width + theta(1:8:40) .* interval(sign_temp);
    theta(3:8:40) = randn(5,1) .* abs(theta(2:8:40)) * width + theta(2:8:40) .* interval(sign_temp);
    theta(4:8:40) = randn(5,1) .* abs(theta(3:8:40)) * width + theta(3:8:40) .* interval(sign_temp);
    theta(5:8:40) = randn(5,1) .* abs(theta(4:8:40)) * width + theta(4:8:40) .* interval(sign_temp);
    theta(6:8:40) = randn(5,1) .* abs(theta(5:8:40)) * width + theta(5:8:40) .* interval(sign_temp);
    theta(7:8:40) = randn(5,1) .* abs(theta(6:8:40)) * width + theta(6:8:40) .* interval(sign_temp);
    theta(8:8:40) = randn(5,1) .* abs(theta(7:8:40)) * width + theta(7:8:40) .* interval(sign_temp);

    %% generate the error on the counts

    theta(25+16) = log(.1+3*rand);
    theta(26+16) = log(2+2*rand);
    theta(27+16) = log(.1+3*rand);
    theta(28+16) = log(2+2*rand);


    %% generate the spliced and unspliced counts

    [~,~,~,N,~,~] = simulate_kinetics_model7(mean_time,theta,[],[],[]);
