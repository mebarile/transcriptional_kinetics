diffGEK for myelopoiesis

Folder run_models: To create the diffGEK input for myelopoiesis, we first create a landscape from the published raw data with the code prepare_ladscape.ipynb. Then, we create a matrix for spliced and unspliced counts per condition for the genes considered with the code prepare_input.ipynb. Since the adatas are difficult to upload, we provide the matrixes directly in the folder data_input.
