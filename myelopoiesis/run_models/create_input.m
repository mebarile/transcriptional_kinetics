%% This code generates the input for modelling the myelopoietic dataset (Ezh2 KO)
...
% D.pop.tm: the mean pseudotime of the cells in condition m (m is the KO)
...
data_w_s = readtable('./data_input/ezh2_wt_ery_spl.csv');
data_w_u = readtable('./data_input/ezh2_wt_ery_unspl.csv');

data_m_s = readtable('./data_input/ezh2_mut_ery_spl.csv');
data_m_u = readtable('./data_input/ezh2_mut_ery_unspl.csv');
...
scale_time = 2.29; % this just scales the dpt to fit in the same range as the model building. 
...
for gene = 1:1228
    ...
    out_name =  ['./data_input/data_gene_' gene_name '.mat'];
