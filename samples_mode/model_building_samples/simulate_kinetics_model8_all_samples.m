% simulate_kinetics_model8_all_samples.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_kinetics_model8_all_samples(tout,theta)
% [...] = simulate_kinetics_model8_all_samples(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_kinetics_model8_all_samples(...)
%
% INPUTS:
% =======
...
function varargout = simulate_kinetics_model8_all_samples(varargin)
...
if(max(options_ami.sens_ind)>64)
...
sol = ami_kinetics_model8_all_samples(tout,theta(1:64),kappa(1:0),options_ami,plist,xscale,init,data);
if(nargout>1)
